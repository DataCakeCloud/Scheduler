...
def sync_perm_for_dag(dag_id, access_control):
    try:
        appbuilder.sm.sync_perm_for_dag(dag_id, access_control)
    except Exception as e:
        flash("Dag:{} permission error, cause: {}".format(dag_id, str(e)), "error")
...
class Airflow(AirflowBaseView):
    ...
    def tree(self, session=None):
        ...
        if not dag:
            ...
            return redirect(url_for('Airflow.index'))
        sync_perm_for_dag(dag_id, dag.access_control)
        root = request.args.get('root')
        if root:
            dag = dag.sub_dag(
                task_regex=root,
    ...
    #  remove @has_dag_access(can_dag_edit=True) for refresh permissions when add dag by owner
    #  fix can_access_control unvalid
...
class ConnectionModelView(AirflowModelView):
    ...
    extra_fields = ['extra__jdbc__drv_path', 'extra__jdbc__drv_clsname',
                    ...
                    'extra__grpc__scopes',
                    'extra__kubernetes_configuration_file_path',
                    'extra__kubernetes_configuration_yaml',
                    'extra__kubernetes_configuration_json'
                    ]
    ...
    def process_form(self, form, is_created):
        formdata = form.data
        if formdata['conn_type'] in ['jdbc', 'google_cloud_platform', 'grpc', 'kubernetes_configuration']:
            extra = {
                key: formdata[key]
