...
from pymysql import OperationalError as pymysql_operational_error
        ...
        if session_in_kwargs or session_in_args:
            ...
            while retries < max_retries:
                with create_session() as session:
                    # Check if the connection is alive with a simple query
                    try:
                        session.execute("SELECT 1")
                    except (OperationalError,pymysql_operational_error) as exp:
                        e = exp
                        # If an exception occurs, try to create a new session
                        retries += 1
                        session.close()
                        continue
            ...
            if e:
                raise e
            raise Exception("Failed to create a valid session after {} retries.".format(max_retries))
