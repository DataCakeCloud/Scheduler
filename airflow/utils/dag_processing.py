...
class SimpleDag(BaseDag):
    ...
    def __init__(self, dag, pickle_id=None):
        ...
        self._max_active_runs = dag.max_active_runs
        self._retry_interval = dag.retry_interval
...
class DagFileProcessorManager(LoggingMixin):
    ...
    def _refresh_dag_dir(self):
        ...
        if elapsed_time_since_refresh > self.dag_dir_list_interval:
            ...
            self._file_paths = TaskDesc.get_all_task_name()
            self.last_dag_dir_refresh_time = now
            self.log.info("There are %s files in %s", len(self._file_paths), self._dag_directory)
            try:
                self.set_file_paths(self._file_paths)
            except Exception:
                self.log.exception("Error set file path!")
    ...
    def heartbeat(self):
        ...
        if len(self._file_path_queue) == 0:
            ...
            self._file_paths.append("DataStudio_PipeLine_RDS")
        ...
        while (self._parallelism - len(self._processors) > 0 and
            ...
            processor.start()
            if file_path == 'spark_nginx_hour_job':
                print('time:{},file_path:{},start proccess,PID: {}'.format(utcnow().strftime('%Y-%m-%d %H:%M:%S'),file_path,processor.pid))
            self.log.debug(
    ...
    def emit_metrics(self):
        ...
        tmp_str = ""
        for stat in self._file_stats.values():
            tmp_str += str(stat.import_errors)
        Stats.gauge('dag_processing.import_errors', tmp_str)
