...
try:
    ...
    if tz == "system":
        ...
        beijing = pendulum.local_timezone()
    else:
        utc = pendulum.timezone(tz)
        beijing = pendulum.timezone(tz)
except Exception:
    pass
...
def utcnow():
    ...
    d = d.replace(tzinfo=pendulum.timezone('UTC'))
    d = d.astimezone(utc)
...
def before_hour():
    """
    Get One hour before current time in UTC
    :return:
    """
    now = utcnow()
...
def my_make_naive(value, timezone=None):
    """
    Make an aware datetime.datetime naive in a given time zone.
