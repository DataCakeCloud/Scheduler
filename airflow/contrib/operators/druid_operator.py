...
class DruidOperator(BaseOperator):
    ...
    :param json_index_file: The filepath to the druid index specification or the json string
    :type json_index_file: str
    :param druid_ingest_conn_id: The connection id of the Druid overlord which
        accepts index jobs
    :param load_json_type: input json type : file or string
    :type load_json_type: str
    ...
    def __init__(self, json_index_file,
        ...
        self.load_json_type = load_json_type

        if load_json_type == 'file':
            with open(json_index_file) as data_file:
                index_spec = json.load(data_file)
        elif load_json_type == 'string':
            try:
                index_spec = json.loads(json_index_file)
            except ValueError as e:
                self.log.error(e)
                raise e
        else:
            raise ValueError("Argument:load_json_type should be 'file'or'string' !")
