...
class KubernetesExecutorConfig:
    ...
    def from_dict(obj):
        ...
        # add base volume
        volume = namespaced.get('volumes', [])
        base_volumes_json = conf.get('kubernetes', 'volumes')
        if base_volumes_json:
            base_volumes = json.loads(base_volumes_json)
        else:
            base_volumes = []
        volume.extend(base_volumes)

        volume_mount = namespaced.get('volume_mounts', [])
        base_volume_mounts_json = conf.get('kubernetes', 'volume_mounts')
        if base_volume_mounts_json:
            base_volume_mounts = json.loads(base_volume_mounts_json)
        else:
            base_volume_mounts = []
        volume_mount.extend(base_volume_mounts)

        return KubernetesExecutorConfig(
            image=namespaced.get('image', None),
            image_pull_policy=namespaced.get('image_pull_policy', None),
            request_memory=namespaced.get('request_memory', conf.get('kubernetes', 'default_request_memory')),
            request_cpu=namespaced.get('request_cpu', conf.get('kubernetes', 'default_request_cpu')),
            limit_memory=namespaced.get('limit_memory', conf.get('kubernetes', 'default_limit_memory')),
            limit_cpu=namespaced.get('limit_cpu', conf.get('kubernetes', 'default_limit_cpu')),
            limit_gpu=namespaced.get('limit_gpu', conf.get('kubernetes', 'default_limit_gpu')),
...
class KubernetesJobWatcher(multiprocessing.Process, LoggingMixin):
    ...
    def _run(self, kube_client, resource_version, worker_uuid, kube_config):
        self.log.info(
            ...
            'worker_uuid: %s', str(resource_version), str(worker_uuid)
        )
        watcher = watch.Watch()
        ...
        for event in watcher.stream(kube_client.list_namespaced_pod, self.namespace,
            ...
            if current_time - last_time > 600:
                print('watcher run for more than ten minutes,refresh')
                break
            task = event['object']
            self.log.info(
            ...
            print('Event: {},{}'.format(task.metadata.name,task.status.phase))
            if event['type'] == 'ERROR':
                return self.process_error(event)
            # get dag info in annotations
            annotations = task.metadata.annotations
            task_instance_related_annotations = {
                'dag_id': annotations['dag_id'] if "dag_id" in annotations.keys() else task.metadata.labels["dag_id"],
                'task_id': annotations['task_id'] if "task_id" in annotations.keys() else task.metadata.labels["task_id"],
                'execution_date': annotations['execution_date'] if "execution_date" in annotations.keys() else task.metadata.labels["execution_date"],
                'try_number': annotations['try_number'] if "try_number" in annotations.keys() else task.metadata.labels["try_number"],
            }
            self.process_status(
    ...
    def process_status(self, pod_id, namespace, status, annotations, resource_version, event):
        """Process status response"""
        if status == 'Pending':
            if event['type'] == 'DELETED':
                self.log.info('Event: Failed to start pod %s, will reschedule', pod_id)
                self.watcher_queue.put((pod_id, namespace, State.UP_FOR_RESCHEDULE, annotations, resource_version))
            else:
...
class AirflowKubernetesScheduler(LoggingMixin):
    ...
    def run_next(self, next_job):
        ...
        TaskInstance.start_taskinstance(dag_id=dag_id,task_id=task_id,execution_date=execution_date)
        # add task life cycle start_time
        TaskLifeCycleHelper.update_start_time(dag_id,execution_date,DateUtil.get_timestamp(timezone.utcnow()))
    ...
    def _annotations_to_key(self, annotations):
        ...
        try:
            ...
            return self._labels_to_key(labels=annotations)
        return (dag_id, task_id, execution_date, try_number)
...
class KubernetesExecutor(BaseExecutor, LoggingMixin):
    ...
    def sync(self):
        ...
        self.log.info("Delete the completed pods and start new pods.")
        ...
        self.log.info("Starting new pods...")
