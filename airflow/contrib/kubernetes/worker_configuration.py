...
class WorkerConfiguration(LoggingMixin):
    ...
    def _get_annotations(self, kube_executor_config, dag_id, task_id, execution_date, try_number):
        annotations = {}
        annotations.update(self.kube_config.kube_annotations)
        annotations.update(dict(kube_executor_config.annotations))
        annotations['dag_id'] = dag_id
        annotations['task_id'] = task_id
        annotations['execution_date'] = execution_date.isoformat()
        annotations['try_number'] = str(try_number)
        return annotations
    ...
    def make_pod(self, namespace, worker_uuid, pod_id, dag_id, task_id, execution_date,
        ...
        annotations = self._get_annotations(kube_executor_config, dag_id, task_id, execution_date, try_number)
        if gcp_sa_key:
