...
class KubernetesRequestFactory:
    ...
    @staticmethod
    def extract_kind(pod,req):
        req["kind"] = pod.kind

    @staticmethod
    def extract_apiVersion(pod,req):
        req["apiVersion"] = pod.api_version
    ...
    @staticmethod
    def extract_preStop(pod, req):
        if pod.preStop_exec_cmds:
            req['spec']['containers'][0]['lifecycle'] = {'preStop':{'exec':{'command':pod.preStop_exec_cmds}}}

    @staticmethod
    def extract_terminationGracePeriodSeconds(pod, req):
        if pod.terminationGracePeriodSeconds:
            req['spec']['terminationGracePeriodSeconds'] = pod.terminationGracePeriodSeconds
