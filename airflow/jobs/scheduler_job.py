...
from airflow.exceptions import AirflowException, TaskNotFound, AirflowTaskTimeout
from airflow.jobs.base_job import BaseJob
from airflow.models import DagRun, SlaMiss, errors, TaskInstance
...
from airflow.ti_deps.dep_context import DepContext, SCHEDULEABLE_STATES, SCHEDULED_DEPS,RUNNING_STATES
from airflow.operators.dummy_operator import DummyOperator
from airflow.ti_deps.deps.pool_slots_available_dep import STATES_TO_COUNT_AS_RUNNING
from airflow.utils import asciiart, helpers, timezone
from airflow.utils.alarm_adapter import send_alarm, get_alarm_group_list
...
class SchedulerJob(BaseJob):
   ...
   :param dag_id: if specified, only schedule tasks with this DAG ID
    ...
    def __init__(
        ...
        self.processor_agent = None
        self.data_pipeline_scheduler_helper = None
    ...
    def _exit_gracefully(self, signum, frame):
        ...
        # if self.data_pipeline_scheduler_helper:
        #     self.data_pipeline_scheduler_helper.end()
        sys.exit(os.EX_OK)
    ...
    def manage_slas(self, dag, session=None):
        ...
        if dag.normalized_schedule_interval is None:
            all_tis = session.query(TI) \
                .with_hint(TI, 'USE INDEX (PRIMARY)', dialect_name='mysql') \
                .filter(TI.dag_id == dag.dag_id) \
                .filter(
                TI.state == State.RUNNING) \
                .filter(TI.task_id.in_(dag.task_ids)).all()
        ...
        if slas:
            ...
            for task in tasks_missed_sla:
                if task.email:
                    ...
                    if task.alarm_to_scmp_groups and task.alarm_severity_on_sla:
                        if isinstance(task.alarm_to_scmp_groups, basestring):
                            alarm_groups |= set(get_alarm_group_list(task.alarm_to_scmp_groups))
                        elif isinstance(task.alarm_to_scmp_groups, (list, tuple)):
                            alarm_groups |= set(task.alarm_to_scmp_groups)
                        alarm_severity = task.alarm_severity_on_sla

            # if alarm_severity_on_sla is on, Send alarm first,and if alarm not success will send email by airflow
            notification_sent = False
            if alarm_groups:
                try:
                    send_alarm(
                        groups=alarm_groups,
                        subject="[airflow] SLA miss on DAG=" + dag.dag_id,
                        message=alarm_content,
                        severity=alarm_severity
                    )
                    notification_sent = True
                except Exception:
                    self.log.exception("Could not send SLA Miss SHAREit Cloud Manager Platform notification for"
                                       " DAG %s", dag.dag_id)

            if emails and not notification_sent:
                try:
                    notification_sent = False
                    send_email(
                        emails,
    ...
    def _process_task_instances(self, dag, task_instances_list, session=None):
        ...
        # 已经在运行的回算dagrun数,key是execution_date
        backfill_dag_run_date_dict = {}
        # 已经在运行的例行dagrun数,key是execution_date
        pipeline_dag_run_date_dict = {}
        for run in dag_runs:
            if run.state == State.RUNNING:
                # 获取所有在调度或running中的ti实例，由此判断实例已在队列中
                tis = run.get_task_instances(state=RUNNING_STATES)
                if tis is not None and len(tis) > 0 and tis[0].execution_date is not None:
                    if run.run_id.startswith("backCalculation"):
                        backfill_dag_run_date_dict[tis[0].execution_date.strftime('%Y%m%d%H%M%S')] = True
                    else:
                        pipeline_dag_run_date_dict[tis[0].execution_date.strftime('%Y%m%d%H%M%S')] = True
                if len(backfill_dag_run_date_dict) + len(pipeline_dag_run_date_dict) >= dag.max_active_runs * 2:
                    break
        ...
        for run in dag_runs:
            ...
            if run.execution_date > timezone.utcnow() and not dag.allow_future_exec_dates:
                ...
                continue
            # 超过十小时发送报警
            tis = run.get_task_instances(task_id=run.dag_id, state=State.RUNNING)
            if tis and tis[0].start_date and tis[0].start_date < timezone.utcnow() - timedelta(hours=10):
                # 是否发过
                from airflow.shareit.models.user_action import UserAction
                user_id = run.dag_id + str(run.execution_date)
                if not UserAction.find_a_user_action(user_id=user_id, action_type="send_10_hour_notify"):
                    # 发提示消息
                    from airflow.shareit.utils.alarm_adapter import get_receiver_list, send_ding_notify,send_email_by_notifyAPI
                    from airflow.shareit.models.task_desc import TaskDesc
                    base_content = """__[DataStudio 任务运行时间超10小时通知]__  
                                __任务: {task_name}__  
                                __执行入参时间: {execution_date}__    
                                任务执行时间以超过10小时，请确认任务状态是否正常"""
                    task = TaskDesc.get_task(task_name=run.dag_id)
                    if task:
                        if isinstance(run.execution_date, datetime):
                            execution_date = run.execution_date.strftime('%Y-%m-%dT%H:%M:%S')
                        else:
                            execution_date = run.execution_date
                        content = base_content.format(task_name=run.dag_id, execution_date=execution_date)
                        notify_type = conf.get('email', 'notify_type')
                        if "email" in notify_type:
                            receiver = get_receiver_list(task.email)
                            send_email_by_notifyAPI(title="DataCake 任务运行时间超10小时通知",content=content,receiver=receiver)
                        if "dingding" in notify_type:
                            receiver = get_receiver_list(task.owner)
                            send_ding_notify(title="DataCake 任务运行时间超10小时通知", content=content, receiver=receiver)
                    # 写入user_action 表
                    UserAction.add_user_action(user_id=user_id, action_type="send_10_hour_notify")
            ...
            if len(backfill_dag_run_date_dict) + len(pipeline_dag_run_date_dict) >= dag.max_active_runs * 2:
                self.log.debug("Dag_id:{} Number of backfill and pipeline active dag runs reached max_active_run * 2.".format(run.dag_id))
                continue

            # 回算和例行调度分开单独计算是否达到了max_active_runs
            if run.run_id.startswith("backCalculation"):
                if len(backfill_dag_run_date_dict) >= dag.max_active_runs:
                    continue
            else:
                if len(pipeline_dag_run_date_dict) >= dag.max_active_runs:
                    continue
            if run.state == State.RUNNING and run.execution_date is not None:
                run_exedate_str = run.execution_date.strftime('%Y%m%d%H%M%S')
                if run_exedate_str not in backfill_dag_run_date_dict.keys() and run_exedate_str not in pipeline_dag_run_date_dict.keys():
                    make_transient(run)
                    active_dag_runs.append(run)
                    if run.run_id.startswith("backCalculation"):
                        backfill_dag_run_date_dict[run_exedate_str] = True
                    else:
                        pipeline_dag_run_date_dict[run_exedate_str] = True
    ...
    def _find_executable_task_instances(self, simple_dag_bag, states, session=None):
        ...
        ti_query = session.query(TI).filter(TI.dag_id.in_(simple_dag_bag.dag_ids))
        ...
        # 判断是否达到并行度，用dag_id做key，因为有部分任务存在2个子task, value是对应正在运行的任务数
        queued_pipeline_ti_dict = {}
        queued_backfill_ti_dict = {}
        # dag_id+date作为key，判断当前实例是否在已运行的队列中，因为有重试这种情况
        queued_pipeline_ti_date_dict = {}
        queued_backfill_ti_date_dict = {}

        for task_instance in task_instances_to_examine:
            # 判断实例的execution_date是否超过当前时间，如果超过了则不加入队列中
            now_date = timezone.utcnow()
            if DateUtil.date_compare(task_instance.execution_date,now_date,eq=False):
                continue
            retry_interval = simple_dag_bag.get_dag(task_instance.dag_id).retry_interval
            # self.log.info("dag_id :{},retry_interval:{},state:{},end_date:{}".format(task_instance.dag_id,str(retry_interval),task_instance.state,
            #                                                                           task_instance.end_date.strftime('%Y-%m-%d %H:%M:%S') if task_instance.end_date else 'NONE'))
            if task_instance.state == State.UP_FOR_RETRY and retry_interval and task_instance.end_date:
                # self.log.info("now_date:{}".format(now_date.strftime("%Y-%m-%d %H:%M:%S")))
                if DateUtil.date_compare(task_instance.end_date + timedelta(seconds=retry_interval), now_date, eq=False):
                    continue

            # 判断实例是否应该运行,因为存在hive2sharestore的任务，有两个子任务，所以要判断子任务能否执行
            task_ids = simple_dag_bag.get_dag(dag_id=task_instance.dag_id).task_ids
            if len(task_ids) > 1 and task_instance.task_id.endswith('_import_sharestore'):
                sharestore_upstream_task_id = None
                for id in task_ids:
                    if id != task_instance.task_id:
                        sharestore_upstream_task_id = id
                # 当前置任务不是成功状态时，不将此任务加入到待执行队列中
                if not TI.check_is_success(task_id=sharestore_upstream_task_id,execution_date=task_instance.execution_date):
                    continue

            max_active = simple_dag_bag.get_dag(task_instance.dag_id).max_active_runs

            ti_key = '{}.{}'.format(task_instance.dag_id,task_instance.execution_date.strftime('%Y%m%d %H%M%S.%f'))
            if task_instance.task_type in PIPELINE_QUEUE:
                # 如果当前dag_id尚未在pipeline_dict中，先刷新一次
                if task_instance.dag_id not in queued_pipeline_ti_dict.keys():
                    self._refresh_queued_ti_nums(queued_pipeline_ti_dict,task_instance.dag_id,PIPELINE_QUEUE,queued_pipeline_ti_date_dict)
                # 判断是否超过了最大运行数
                if task_instance.dag_id in queued_pipeline_ti_dict.keys():
                    if ti_key not in queued_pipeline_ti_date_dict.keys() and queued_pipeline_ti_dict[task_instance.dag_id] >= max_active:
                        continue
                    elif ti_key not in queued_pipeline_ti_date_dict.keys():
                        queued_pipeline_ti_dict[task_instance.dag_id] = queued_pipeline_ti_dict[task_instance.dag_id] + 1
                        queued_pipeline_ti_date_dict[ti_key] = True
                else:
                    queued_pipeline_ti_dict[task_instance.dag_id] = 1
                    queued_pipeline_ti_date_dict[ti_key] = True

            if task_instance.task_type in BACKFILL_QUEUE:
                # 如果当前dag_id尚未在backfill_dict中，先刷新一次
                if task_instance.dag_id not in queued_backfill_ti_dict.keys():
                    self._refresh_queued_ti_nums(queued_backfill_ti_dict,task_instance.dag_id,BACKFILL_QUEUE,queued_backfill_ti_date_dict)
                # 判断是否超过了最大运行数
                if task_instance.dag_id in queued_backfill_ti_dict.keys():
                    if ti_key not in queued_backfill_ti_date_dict.keys() and queued_backfill_ti_dict[task_instance.dag_id] >= max_active:
                        continue
                    elif ti_key not in queued_backfill_ti_date_dict.keys():
                        queued_backfill_ti_dict[task_instance.dag_id] = queued_backfill_ti_dict[task_instance.dag_id] + 1
                        queued_backfill_ti_date_dict[ti_key] = True
                else:
                    queued_backfill_ti_dict[task_instance.dag_id] = 1
                    queued_backfill_ti_date_dict[ti_key] = True
    ...
    def _change_state_for_executable_task_instances(self, task_instances,
                                                    acceptable_states, simple_dag_bag ,session=None):
        """
    ...
    @provide_session
    def _refresh_queued_ti_nums(self,ti_dict,ti_dag_id,ti_task_types,ti_date_dict,session=None):
        TI = models.TaskInstance
        queued_tis = session.query(TI).filter(TI.dag_id == ti_dag_id).filter(
            TI.state.in_(State.queued())).all()
        for ti in queued_tis:
            if ti.task_type in ti_task_types:
                ti_dict[ti.dag_id] = 1 if ti.dag_id not in ti_dict.keys() else \
                    ti_dict[ti.dag_id] + 1
                ti_date_dict['{}.{}'.format(ti_dag_id,ti.execution_date.strftime('%Y%m%d %H%M%S.%f'))] = True
    ...
    def _change_state_for_tasks_failed_to_execute(self, session):
        ...
        if self.executor.queued_tasks:
            TI = models.TaskInstance
            '''
            查出所有在queued_task中的实例，并且仍然是queued状态的，目的是把这些任务重新置为sheduled，以便于下一次调度
            因为在一次心跳过后，已经把所有可以执行的实例状态更新了，下次会有新的queued_task
            '''
            filter_for_ti_state_change = (
                [and_(
                    TI.dag_id == dag_id,
    ...
    def _execute(self):
        ...
        from airflow.shareit.models.task_desc import TaskDesc
        known_file_paths = TaskDesc.get_all_task_name()
        ...
        #                                              processor_timeout,
        #                                              async_mode)
        self.data_pipeline_scheduler_helper = SchedulerManager()
    ...
    def _execute_helper(self):
        ...
        # self.reset_state_for_queued_tasks()

        # Start after resetting orphaned tasks to avoid stressing out DB.
        # self.processor_agent.start()
        # self.data_pipeline_scheduler_helper.start()
        ...
        while (timezone.utcnow() - execute_start_time).total_seconds() < \
            ...
            try:
                with timeout(3):
                    monitor_push.push_to_prometheus(job_name="scheduler_job", gauge_name="scheduler_job",
                                                    documentation="scheduler_job")
            except AirflowTaskTimeout as e:
                self.log.error("[DataStudio Pipeline] push metrics timeout")
            except Exception as e:
                self.log.error("[DataStudio Pipeline] scheduler_job push metrics failed")
            ...
            # dag_ids = TaskService.get_all_running_dag_id()
            try:
                dag_ids = TaskInstance.list_all_scheduled_task()
            except Exception as e:
                s = traceback.format_exc()
                self.log.error(s)
                continue
            myDagBag = DagBag(dag_folder_list=dag_ids)
            dags = myDagBag.dags
            simple_dags = []
            for dag_id in dags:
                dag = dags[dag_id]
                if dag and dag.dag_id in dag_ids:
                    simple_dags.append(SimpleDag(dags[dag_id]))

            self.log.info("[DataStudio Pipeline] DAG collected !")
            self.log.info("[DataStudio Pipeline] Tasks to be performed, tasks : {}".format(','.join(dag_ids)))
            # tmp = self._get_simple_dags()
            # if tmp is not None and isinstance(tmp, list):
            #     # simple_dags.extend(tmp)
            #     pass
            # self.log.debug("Harvested {} SimpleDAGs".format(len(simple_dags)))

            # Send tasks for execution if available
            simple_dag_bag = SimpleDagBag(simple_dags)
            self.log.info("[DataStudio Pipeline] Start run taskInstance ")
            if not self._validate_and_run_task_instances(simple_dag_bag=simple_dag_bag):
                continue
            self.log.info("[DataStudio Pipeline] End ")
            ...
            if loop_duration < 5 and not is_unit_test:
        ...
        # self.data_pipeline_scheduler_helper.terminate()
    ...
    def process_file(self, file_path, zombies, pickle_dags=False, session=None):
        ...
        try:
            ...
            self.log.debug("[DataStudio pipeline] dagbag :" + str(dagbag.dag_ids))
        except Exception:
