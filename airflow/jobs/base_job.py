...
class BaseJob(Base, LoggingMixin):
    ...
    @provide_session
    def reset_state_for_queued_tasks(self,session=None):
        """
        每次启动前，将queued状态任务重置为scheduled
        """
        TI = models.TaskInstance
        tis = session.query(TI).filter(TI.task_id != 'createCheckFile').filter(TI.state == State.QUEUED).all()
