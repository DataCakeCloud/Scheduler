...
class LocalTaskJob(BaseJob):
            ...
            while True:
                ...
                if return_code is not None:
                    ...
                    try:
                        ...
                        dag_folder = conf.get('core', 'base_log_folder')
                        # 本地日志文件
                        app_log_file = '{dag_folder}/{dag_id}/{task_id}/{time}/{try_number}.log'.format(
                            dag_folder=dag_folder,
                            dag_id=self.task_instance.dag_id,
                            task_id=self.task_instance.task_id,
                            time=self.task_instance.execution_date.isoformat(),
                            try_number=self.task_instance.try_number - 1 if self.task_instance.try_number > 0 else 0)
                        ...
                        # 云存储的文件
                        log_cloud_file = '{log_cloud_home}/BDP/logs/schedule_log/{env}/{dag_id}/{time}/{try_number}.log'.format(
                        ...
                        self.log.error("upload log to cloud error")
                        self.log.error(traceback.format_exc())
                    if return_code == -9:
                        try:
                            self.task_instance.state=State.FAILED
                            TriggerHelper.task_finish_trigger(self.task_instance)
                        except AirflowException as e:
                            self.log.error("任务因为资源不足失败，恢复实例状态失败！")
    ...
    def heartbeat_callback(self, session=None):
        ...
        if ti.state == State.RUNNING:
            ...
            time.sleep(5) # 这里sleep5s,防止task_runner还有些逻辑未执行完
            self.task_runner.terminate()
