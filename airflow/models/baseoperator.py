...
class BaseOperator(LoggingMixin):
    ...
    :param alarm_to_scmp_groups: select the groups to send the alarm
    :type alarm_to_scmp_groups: str or list[str]
    :param alarm_to_scmp_on_retry: Indicates whether alerts should send to scmp when a
        task is retried
    :type alarm_to_scmp_on_failure: bool
    :param alarm_on_failure: Indicates whether email alerts should send to scmp when
        a task failed
    :param alarm_severity_on_failure:
                specify the alarm level when a task fails
    ...
    :type alarm_severity_on_failure: str
    :param alarm_severity_on_retry:
                specify the alarm level when a task retry
                critical: alarm to email, dingding group ,dingding work notification and on call
                emergency: alarm to email, dingding work notification and dingding group
                warning: alarm to email only
                cite https://doc.ushareit.me/web/#/4?page_id=1483
    :type alarm_severity_on_retry: str
    :param alarm_severity_on_sla:
                specify the alarm level when a task miss its sla
                critical: alarm to email, dingding group ,dingding work notification and on call
                emergency: alarm to email, dingding work notification and dingding group
                warning: alarm to email only
                cite https://doc.ushareit.me/web/#/4?page_id=1483
    :type alarm_severity_on_sla: str
    ...
    _comps = {
        ...
        'email',
        'email_on_retry',
        'alarm_to_scmp_on_retry',
        'alarm_to_scmp_groups',
        'retry_delay',
    ...
    def __init__(
        ...
        dingAlert=True,  # type: bool
        phoneAlert=False,  # type: bool
        email=None,  # type: Optional[Union[str, Iterable[str]]]
        email_on_retry=False,  # type: bool
        email_on_failure=False,  # type: bool
        email_on_success=False,  # type: bool
        email_on_start=False,   # type: bool
        alarm_to_scmp_groups=None,  # type: Optional[Union[str, Iterable[str]]]
        alarm_to_scmp_on_failure=True,  # type: bool
        alarm_severity_on_failure='EMERGENCY',  # type: str
        alarm_to_scmp_on_retry=False,  # type: bool
        alarm_severity_on_retry='WARNING',  # type: str
        ...
        alarm_severity_on_sla='WARNING',  # type: str
        ...
        **kwargs
        ...
        self.owner = owner
        self.email = email
        self.email_on_start = email_on_start
        self.email_on_retry = email_on_retry
        self.email_on_failure = email_on_failure
        self.email_on_success = email_on_success
        self.alarm_to_scmp_groups = alarm_to_scmp_groups
        self.alarm_to_scmp_on_failure = alarm_to_scmp_on_failure
        self.alarm_to_scmp_on_retry = alarm_to_scmp_on_retry
        self.alarm_severity_on_failure = alarm_severity_on_failure
        self.alarm_severity_on_retry = alarm_severity_on_retry
        self.alarm_severity_on_sla = alarm_severity_on_sla
        self.dingAlert = dingAlert
        self.phoneAlert = phoneAlert
        self.notified_owner = notified_owner
    ...
    def pre_execute(self, context):
        ...
        td = TaskDesc()
        ts = TaskService()
        cloud_path = td.get_success_dir(task_name=self.task_id)
        print cloud_path
        if cloud_path:
            cloud_path = ts.render_api(cloud_path, context, task_name=self.task_id)
            self.log.info("------------检验文件路径是否有权限------------")
            pattern = re.compile(r'[\s{}#*()]')
            res = pattern.findall(cloud_path.strip())
            if(len(res)==0):
                tenant_id, cluster_region, provider = TaskDesc.get_cluster_manager_info(task_name=self.dag_id)
                cloud = CloudUtil(provider=provider, region=cluster_region, tenant_id=tenant_id)
                # cloud = CloudUtil()
                cloud.list_object(cloud_path)
            else:
                raise Exception("文件格式不对"+str(cloud_path))
    ...
    def post_execute(self, context, result=None):
        ...
        self._log.debug("------------运行post_execute------------")
        from airflow.shareit.models.task_desc import TaskDesc
        from airflow.shareit.service.task_service import TaskService
        import re
        td = TaskDesc()
        ts = TaskService()
        file_name = td.get_success_file(task_name=self.task_id)
        if file_name:
            file_name = ts.render_api(file_name, context, task_name=self.task_id)
            self._log.debug(file_name)
            print file_name
            ...
            try:
                ...
                self.log.error('上传成功标志失败')
                self.log.error(traceback.format_exc())
                raise
            self.log.info("成功标志已生成")
