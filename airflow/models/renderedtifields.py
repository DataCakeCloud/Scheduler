...
class RenderedTaskInstanceFields(Base):
    ...
    def delete_old_records(
        ...
        if num_to_keep <= 0:
            return
        num_rows = session \
            .query(cls.execution_date) \
            .filter(cls.dag_id == dag_id, cls.task_id == task_id) \
            .count()
        if num_rows <= num_to_keep:
            return

        delete_execution_date = session \
            .query(cls.execution_date) \
            .filter(cls.dag_id == dag_id, cls.task_id == task_id) \
            .order_by(cls.execution_date.asc()) \
            .limit(num_rows - num_to_keep) \
            .all()
        ...
        if session.bind.dialect.name in ["postgresql", "sqlite"]:
            ...
            #     .delete(synchronize_session=False)
            for delete_date in delete_execution_date:
                session.query(cls) \
                    .filter(cls.dag_id == dag_id,
                            cls.task_id == task_id,
                            cls.execution_date == delete_date[0])\
                    .delete(synchronize_session=False)
        else:
            # Fetch Top X records given dag_id & task_id ordered by Execution Date
