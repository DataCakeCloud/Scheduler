...
class DAG(BaseDag, LoggingMixin):
    ...
    def __init__(
            ...
            schedule_interval=None,  # type: Optional[ScheduleInterval]
    ...
    @provide_session
    def sync_to_db_transaction(self, owner=None, sync_time=None,is_online=False, transaction=False,session=None):
        """
...
class DagModel(Base):
    ...
    def get_task_owner(task_id):
        dag = DagModel.get_dagmodel(task_id)
        return dag.owners if dag else ''
    ...
    @staticmethod
    @provide_session
    def is_active_and_on(dag_id=None, session=None):
        if dag_id is None:
            return False
        all_active_dags = session.query(DagModel).filter(DagModel.is_active == True,
                                                         DagModel.is_paused == False,
                                                         DagModel.dag_id == dag_id).all()
        if all_active_dags:
            return True
        else:
            return False
    ...
    def update_name(old_name,new_name,transaction=False,session=None):
        dagmodel = session.query(DagModel).filter(DagModel.dag_id == old_name).first()
        if dagmodel:
            dagmodel.dag_id = new_name
    ...
    @staticmethod
    @provide_session
    def list_all_online_task(session=None):
        all_active_dags = session.query(DagModel).filter(DagModel.is_active == True,
                                                         DagModel.is_paused == False).all()
        return all_active_dags