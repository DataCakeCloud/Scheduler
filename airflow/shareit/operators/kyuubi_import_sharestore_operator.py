...
class KyuubiImportSharestoreOperator(KyuubiPodOperator):
    """
    ...
    def __init__(self,
        ...
        :param kwargs:
        """
        super(KyuubiImportSharestoreOperator, self).__init__(*args, **kwargs)
        ...
        if input_path:
            ...
            if cluster_region == 'us-east-1':
                cluster_region = 'ue1'
            elif cluster_region == 'ap-southeast-1':
                cluster_region = 'sg1'
            elif cluster_region == 'ap-southeast-3':
                cluster_region = 'sg2'
    ...
    def my_init(self):
        self.truncate_string() #sharestore任务名称有可能超过63位，截取一下
        ...
        try:
            ...
            if self.cluster_region == 'us-east-1':
                cluster_region = 'ue1'
            elif self.cluster_region == 'ap-southeast-1':
                cluster_region = 'sg1'
            elif self.cluster_region == 'ap-southeast-3':
                cluster_region = 'sg2'
            self.pool = '{}.{}'.format(self.cluster_load,cluster_region) if cluster_region else self.cluster_load
            self.retry_delay = timedelta(seconds=release_lock_delay_sec + 180)
            self.terminationGracePeriodSeconds = terminationGracePeriodSeconds
    ...
    def truncate_string(self):
        """
        如果输入字符串超过63个字符，从后面截断，返回截断后的字符串
        """
        if len(self.job_name) > 63:
            self.job_name = self.job_name[:63]
            if self.job_name.endswith('_'):
                self.job_name = self.job_name.rstrip('_')
