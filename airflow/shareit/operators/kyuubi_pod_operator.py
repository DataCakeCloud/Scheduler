...
conf.get
class KyuubiPodOperator(BaseOperator):
    """
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(KyuubiPodOperator, self).__init__(*args, **kwargs)
        self.image = image
        ...
        try:
            ...
            with TemporaryDirectory(prefix='airflowtmp') as tmp_dir:
                ...
                with open(os.path.join(tmp_dir, tmp_file_name), 'w') as tmp_file:
                    ...
                    file_location = self.upload_yaml(location)
                    self.log.info('file location: %s', file_location)
                    dependencies.append(file_location)
                    print file_location
    ...
    def upload_yaml(self,file):
        ...
        artifact_home = 's3://shareit.deploy.ap-southeast-1/BDP/BDP-kyuubi/'
        date = DateUtil.format_millisecond(timezone.utcnow().replace(tzinfo=beijing))
        ts_date = DateUtil.get_timestamp(date)
        cloud_file_path = '{location}/yaml/{yaml}'.format(location=artifact_home.rstrip('/'),yaml= '{}_{}.yaml'.format(self.task_id,str(ts_date)))
        ...
        # cloud = CloudUtil()
        cloud.upload_to_cloud(cloud_file_path,file)
